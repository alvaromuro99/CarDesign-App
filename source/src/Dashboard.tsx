...
export default function Dashboard({ onOpen }: { onOpen: (v: string) => void }) {
  ...
  const kcard = (chip: string, chipBg: string, label: string, value: string, color: string, foot: React.ReactNode) => (
    <div className="kcard"><div className="ktop"><span className="kchip" style={{ background: chipBg }}>{chip}</span><span className="klabel">{label}</span></div>
  ...
  return (<div className="boardwrap dash"><div className="topbar"><span className="crumb">🏠 Inicio</span></div>
    <div className="finwrap">
      ...
      <div className="kpi6">
        {kcard('📈', 'rgba(62,196,109,.16)', 'Beneficio', eur(fin.neto), fin.neto >= 0 ? '#3ec46d' : '#e63946', finPrev ? trend(fin.neto, finPrev.neto, true) : <span className="kt-sub">{eur(fin.ingresos)} − {eur(fin.gastos)}</span>)}
        {kcard('👥', 'rgba(155,93,229,.16)', 'Seguidores totales', num(followers), '#9b5de5', trend(followers, followersPrev))}
        {kcard('👁️', 'rgba(79,124,255,.16)', 'Visualizaciones (redes)', num(visitsSocial), '#4f7cff', trend(visitsSocial, visitsSocialPrev))}
        {kcard('🌐', 'rgba(0,184,217,.16)', 'Páginas vistas web', num(visitsWeb), '#00b8d9', trend(visitsWeb, visitsWebPrev))}
        {kcard('📘', 'rgba(244,166,35,.16)', 'Anuarios entregados', num(fin.anuarios), 'var(--txt)', <span className="klink" onClick={(e) => { e.stopPropagation(); onOpen('finances'); }}>Ver finanzas →</span>)}
        {kcard('✅', 'rgba(244,166,35,.16)', 'Tareas', doing + ' en curso', '#f4a623', overdue.length ? <span className="klink" style={{ color: '#e63946' }} onClick={(e) => { e.stopPropagation(); onOpen('board'); }}>{overdue.length} vencidas →</span> : <span className="klink" onClick={(e) => { e.stopPropagation(); onOpen('board'); }}>Ver mis tareas →</span>)}
      </div>

      <div className="finrow3">
        <div className="dbox"><div className="dbox-h">💰 Finanzas del periodo</div>
        ...
        <div className="dbox"><div className="dbox-h">🏆 Tareas {overdue.length ? <span className="badge-red">{overdue.length} vencidas</span> : ''}</div>
        ...
        <div className="dbox"><div className="dbox-h">📅 Próximos eventos</div>
