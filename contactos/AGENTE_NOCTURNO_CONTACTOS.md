...
- **TIENES MUCHA MÁS CAPACIDAD DE LA QUE CREES: repite el proceso en bucle 7-9 veces seguidas. POR DEFECTO HAZ 8 RONDAS.** La experiencia real demuestra que en una sola sesión caben holgadamente **7-9 rondas completas** de Trabajo 2 + 3 + 4 con su despliegue (≈24 verificaciones por ronda; ~192 verificaciones con 8 rondas). No te quedes corto: encadena Ronda 1 → guardar+desplegar → Ronda 2 → guardar+desplegar → … hasta **8 rondas** (sube a 9 si vas sobrado, baja solo si de verdad te acercas al límite de contexto). Mantén los lotes pequeños dentro de cada ronda (búsquedas web de 3 en 3, aplicar cambios con un único script Python en `/tmp/r.json`, validar 13 columnas antes de guardar), pero **repite el ciclo 8 veces** en vez de parar pronto.
- **Eficiencia para aguantar 8 rondas:** no vuelques resultados largos al contexto innecesariamente; reutiliza el mismo patrón cada ronda (sacar candidatos desde `verify_index` → 8 lotes de 3 WebSearch → volcar veredictos a `/tmp/r.json` → aplicar T2 con un script → escaneo de duplicados T3 → 1-2 rellenos T4 → guardar → desplegar por Chrome). Avanza `verify_index`/`fill_index` solo por lo procesado; las notas evitan reprocesar.
