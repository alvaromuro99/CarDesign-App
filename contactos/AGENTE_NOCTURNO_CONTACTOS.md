...
- **TIENES MÁS CAPACIDAD DE LA QUE CREES: repite el proceso en bucle 3 veces seguidas.** La experiencia real demuestra que en una sola sesión caben holgadamente **3 rondas completas** de Trabajo 2 + 3 + 4 con su despliegue (≈24 verificaciones por ronda, ~72 en total). No te quedes en una sola pasada: encadena Ronda 1 → guardar+desplegar → Ronda 2 → guardar+desplegar → Ronda 3 → guardar+desplegar. Mantén los lotes pequeños dentro de cada ronda (búsquedas web de 3 en 3, aplicar cambios con un único script Python, validar 13 columnas antes de guardar), pero **repite el ciclo varias veces** en vez de parar pronto.
